	...
	, launchaText("Launch", 24, glm::vec4(1.0f))
{
    ...
    for(int i = 0; i < dirList.GetFilecount(); i++)
    {
        ...
        homebrewButtons[idx].nameLabel = new GuiText(cpName, 42, glm::vec4(1.0f));
		homebrewButtons[idx].nameLabel->setColor(glm::vec4(0.1f, 0.1f, 0.1f, 1.0f));
        homebrewButtons[idx].nameLabel->setAlignment(ALIGN_MIDDLE | ALIGN_MIDDLE);
        ...
        homebrewButtons[idx].descriptionLabel = new GuiText(cpDescription, 24, glm::vec4(1.0f));
        homebrewButtons[idx].descriptionLabel->setColor(glm::vec4(0.1f, 0.1f, 0.1f, 1.0f));
    ...
    hblVersionText.setColor(glm::vec4(0.5f, 0.5f, 0.5f, 1.0f));
    hblVersionText.setAlignment(ALIGN_TOP | ALIGN_LEFT);
    hblVersionText.setPosition(180, -67);
	
	launchaText.setColor(glm::vec4(0.1f, 0.1f, 0.1f, 1.0f));
	launchaText.setAlignment(ALIGN_BOTTOM | ALIGN_RIGHT);
	launchaText.setPosition(-37, 52);
...
void HomebrewWindow::process()
{
    OSCalendarTime cal_time;
	OSTicksToCalendarTime(OSGetTime(), &cal_time);
	time.setTextf("%02i:%02i:%02i PM 03/%02i/%04i", cal_time.hour, cal_time.min, cal_time.sec, cal_time.mday, cal_time.year);
